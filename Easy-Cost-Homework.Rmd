...
Median imputation will enable similar results to mean imputation (but with less sensitivity to outliers vs. mean). Single imputation with the medians will preserve the median but 
since the variability in the data will be reduced, the standard deviations and the variance estimates will tend to be underestimated. The magnitude of the covariances and correlation also decrease by restricting the variability. Thus the variance and correlation structure will be distorted. 
...
The latter consists in imputing with the predicted values from a regression model plus a random noise drawn from a normal distribution with variance equal to the residual variance.